...
- **历史记录**：每次保存的结算记录共享给房间内所有人（含具体抄表读数、电费费用明细及备注），支持回填/删除；回填时该记录的"结束日期"会自动填入当前周期的"开始日期"
- **自动填充**：进入房间时，按费用种类分别把最近一次历史的"本次抄表数"填入"上次抄表数"，把该条历史的"结束日期"填入对应周期的"开始日期"，并恢复最近使用的住户昵称；"本次抄表数"和电费费用项是各账期不同的临时输入，不会自动填充
