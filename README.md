...
- **历史记录**：每次保存的结算记录共享给房间内所有人（含具体抄表读数），支持回填/删除
- **自动填充**：进入房间时，按费用种类分别把最近一次历史的"本次抄表数"填入"上次抄表数"，把该条历史的"结束日期"填入对应周期的"开始日期"，并恢复最近使用的住户昵称；"本次抄表数"是未保存的临时输入，刷新页面不会回填
...
### 用户体验
- 住户可自定义昵称（按房间独立记忆，保存记录时取当前昵称快照），所有标题和结果同步更新
- 电费/水费计费周期相互独立：电费 Tab 只显示电费周期，水费 Tab 只显示水费周期，结束日期默认今天
- 数字输入框无上下增减箭头、滚轮不改值，避免误触改数
...
4. 在 **👥 住户设置** 里填写住户昵称，在 **📅 计费周期设置** 里设置对应费用种类的周期（电费/水费 Tab 各自显示自己的周期）
